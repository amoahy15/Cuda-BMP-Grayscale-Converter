...
__global__ void color_to_grey(uchar3 *input_image, uchar3 *output_image, int width, int height)
{
    ...
    if (y < width && x < height)
    {
        uchar3 color = input_image[offset];
        float R = color.x;
        float G = color.y;
        float B = color.z;

        unsigned char intensity = (unsigned char)(0.299f * R + 0.578f * G + 0.114f * B);
        output_image[offset].x = intensity;
        output_image[offset].y = intensity;
        output_image[offset].z = intensity;
    }
}
